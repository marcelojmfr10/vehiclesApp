...
class _VehicleScreenState extends State<VehicleScreen> {
  ...
  List<DropdownMenuItem<int>> _getComboVehicleTypes() {
    ...
    _vehicleTypes.forEach((vehicleType) {
      list.add(DropdownMenuItem(
          child: Text(vehicleType.description), value: vehicleType.id));
    });
    return list;
  }
  ...
  List<DropdownMenuItem<int>> _getComboBrands() {
    ...
    _brands.forEach((brand) {
      list.add(
          DropdownMenuItem(child: Text(brand.description), value: brand.id));
    });
    return list;
  }
  ...
  void _goAddPhoto() async {
    var response = await showAlertDialog(
        ...
        actions: <AlertDialogAction>[
          ...
          AlertDialogAction(key: 'gallery', label: 'Imágenes')
        ]);
  ...
  void _confirmDeletePhoto() async {
    var response = await showAlertDialog(
        context: context,
        title: 'Confirmación',
        message: '¿Estás seguro de querer borrar la última foto tomada?',
        actions: <AlertDialogAction>[
          AlertDialogAction(key: 'no', label: 'No'),
          AlertDialogAction(key: 'yes', label: 'Sí')
        ]);
